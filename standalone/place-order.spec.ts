...
//
// Self-contained file — no config, fixtures, or helper files needed.
// Run: npm i -D @playwright/test && npx playwright install chromium && npx playwright test

// The demo site is a slow shared host; give every assertion more headroom.
...
const USER = {
  name: 'Sage QA',
  email: `sage.qa.${Date.now()}@example.com`, // unique per run
};
...
test.beforeEach(({ page }) =>
  page.route('**/*', (route) =>
    AD_HOSTS.some((host) => route.request().url().includes(host))
      ? route.abort()
      : route.continue(),
  ),
);
...
test.describe('Place Order: Register before Checkout (Test Case 15)', () => {
  test('TC15 — end-to-end purchase flow', async ({ page }) => {
    ...
    await test.step(`Add ${PRODUCTS_TO_ADD} products to the cart and verify the count`, async () => {
      ...
      await expect(page.locator('#cart_info_table tbody tr[id^="product-"]')).toHaveCount(
        PRODUCTS_TO_ADD,
      );
    });
    ...
    await test.step('Check out — the delivery address must match the registration data', async () => {
      ...
      await expect(page.locator('#cart_info tbody tr[id^="product-"]')).toHaveCount(
        PRODUCTS_TO_ADD,
      );
    });
